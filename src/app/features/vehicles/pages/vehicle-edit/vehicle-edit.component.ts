        ...
        <form [formGroup]="vehicleForm" (ngSubmit)="onSubmit()">
          <div class="form-grid">
            ...
            <app-form-field
              ...
              [required]="true"
            >
              <option value="Minivan">Minivan (Küçük Boy)</option>
              <option value="Panelvan">Panelvan (Orta Boy)</option>
              <option value="Kamyonet">Kamyonet (Büyük Boy)</option>
            </app-form-field>
            ...
            <app-form-field
              label="Atanmış Sürücü / Teknisyen"
              type="select"
              [control]="$any(vehicleForm.get('assignedTechnicianId'))"
            >
              <option [value]="null">Atama Yapma (Boşta)</option>
              <option *ngFor="let t of technicians" [value]="t.id">{{ t.fullName }} ({{ getBranchName(t.branchId) }})</option>
            </app-form-field>
          ...
          <div class="form-group full-width" style="margin-top: 1rem;">
            <label for="equipmentsInput">Araç İçi Ekipman Seti / Demirbaşlar (Virgülle ayırarak yazın)</label>
            <input id="equipmentsInput" type="text" [value]="equipmentsJoined" />
            <span class="hint-text">Örn: Merdiven, Jeneratör, Alet Çantası, Matkap</span>
          </div>
...
export class VehicleEditPage implements OnInit, ComponentCanDeactivate {
  ...
  vehicleForm: FormGroup = this.fb.group({
    plateNumber: ['', [Validators.required, Validators.maxLength(FIELD_LIMITS.plate), CustomValidators.plateNumber()]],
    ...
    lastMaintenanceDate: ['', [Validators.required, CustomValidators.dateRange(DATE_LIMITS.PAST_MIN, todayISO())]],
    isActive: [true]
  });
  ...
  ngOnInit(): void {
    try {
      ...
      if (id) {
        ...
        if (found) {
          this.vehicle = found;
          this.equipmentsJoined = (found.equipments || []).join(', ');
          this.vehicleForm.patchValue({
            ...found,
            lastMaintenanceDate: found.lastMaintenanceDate ? found.lastMaintenanceDate.substring(0, 10) : ''
          });
        } else {
  ...
  async onSubmit(): Promise<void> {
    if (this.vehicleForm.valid && this.vehicle) {
      ...
      const approved = await this.confirmService.confirm(
        'Araç Düzenleme',
        'Araç değişikliklerini kaydetmek istediğinize emin misiniz?'
      );
      if (!approved) return;
      ...
      try {
        ...
        this.toastService.showSuccess('Araç başarıyla güncellendi.');
